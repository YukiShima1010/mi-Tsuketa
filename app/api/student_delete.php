...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 値を確認
    $student_id = isset($post_data["student_id"]) ? $post_data["student_id"] : false;

    if (!$student_id || !is_array($student_id)) {
        // 値が正しくなかった場合
        $response["status"] = 500;
        $response["error_message"] = "POSTの値が不正です。";
    } else {
        try {
            $select_ids = implode(", ", array_fill(0, count($student_id), "?"));

            // 使用するSQLを選ぶ
            $select = isset($_POST["select"]) ? $_POST["select"] : false;
            if (!$select || $select == 1) {
                // 選択中
                $sql = "DELETE FROM students WHERE id IN ($select_ids)";
            } elseif ($_POST["select"] == 2) {
                // 選択以外
                $sql = "DELETE FROM students WHERE id NOT IN ($select_ids)";
            } else {
                $response["status"] = 500;
                $response["error_message"] = "POSTの値が不正です。";
